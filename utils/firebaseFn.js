...
const metadata = {
  ...
  customMetadata: {
    userid: userid,  // Add custom metadata (e.g., user ID)
    description: "User uploaded image",  // Any additional custom fields
    uploadedAt: new Date().toISOString()  // Add timestamp as metadata
  }
};
  try {
    const snapshot = await uploadBytes(storageRef, file.buffer,metadata);
    console.log(snapshot, "this goes in hahahah")
  ...
  async function listAllImagesWithMetadata(directoryPath = 'images/') {
   ...
   module.exports = {uploadToFirebase , listAllImagesWithMetadata}